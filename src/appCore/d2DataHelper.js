    ...
    */
    mergeMetaData: (newData) => {
        ...
        for (let i = 0; i < newData.length; i++) {
            const metaData = newData[i].metaData;
    ...
    mergeAnalyticsResults: (oldData, dataToMerge) => {
        ...
        for (let i = 0; i < dataToMerge.length; i++) {
            const header = dataToMerge[i].headers;
            const metaData = dataToMerge[i].metaData;
            const rows = dataToMerge[i].rows;

            let dxi = null, pei = null, oui = null, coi = null, vali = null, ati = null;
